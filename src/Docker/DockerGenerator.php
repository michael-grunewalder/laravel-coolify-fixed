...
http {
    ...
    # Don't advertise the nginx version on the Server header / error pages.
    server_tokens off;
    ...
    server {
        ...
        # `always` so the headers also ride on 4xx/5xx responses.
...
{$extraLocationStr}
        ...
        # Commodity scanner probes — WordPress/RCE paths that can never be
        # legitimate on an app generated by this package. Refused here so they
        # don't boot PHP; also matched before the generic \\.php\$ handler.
        location ~* (^/(wp-login\\.php|xmlrpc\\.php|app_dev\\.php)|^/wp-admin(/|\$)|eval-stdin\\.php\$) {
            deny all;
        }
